          ...
          this.transaction(pkt).then(async (rslt) => { // eslint-disable-line
            if (typeof (rslt) === 'undefined') {
              ...
              if (rslt[6] === this.EX_LOCK_FAIL) {
                ...
                setupRetry -= 1;
                if (setupRetry === 0) {
                  reject('Device is setting up.');
                } else {
                  await sleep(this.SETUP_SLEEP);
                }
              } else {
                reject(`Unknown Error: ${rslt[6]}`);
              }
            } else {
              reject(`Device return unexpected response: ${rslt[5]}`);
            }
            resolve(handle());
          });
        });
